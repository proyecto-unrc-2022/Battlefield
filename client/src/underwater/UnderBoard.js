...
export default function UnderBoard({ visibleState, height, width }) {
  ...
  function getVisibility() {
    if (visibleState === null || visibleState.visible_board === undefined) return;
  ...
  return (
    <div className={`u-grid-${width}`}>
      {
        board.map((row, i) => row.map((col, j) => (<UnderCell visibleState={visibleState} key={(i + 1) * (j + 1)} typeString={col} images={images} />)))
      }
    </div>
  );
}
