...
export default function UnderGame() {
  ...
  useEffect((_) => {
    ...
    sse.onmessage = ((e) => {
      ...
      else if (data.message === 'moved' || data.message === 'submarine placed' || data.message === 'torpedos moved') updateVisibleState();
  ...
  useEffect((_) => {
    if (winner !== null) setLayout({ main: <ShowWinner winner={winner} />, bottom: null });
    else if (roomIsFull) {
      if (chosenSubmarine == null) {
        ...
        setLayout({
          ...
          bottom: <>
            <UnderStats visibleState={visibleState} currentUserId={currentUserId} />
