...
class ActionRequestValidator(Schema):
    ...
    def check_game(self, in_data, **kwargs):
        ...
        navy_game_user: NavyGame = db.session.query(NavyGame).filter(and_(NavyGame.id == navy_game_id,
        (or_(NavyGame.user1_id == user_id, NavyGame.user2_id == user_id)))).first()
    ...
    def check_ship(self, in_data, **kwargs):
        ...
        elif ship.user_id != in_data.get("user_id") or ship.navy_game_id != in_data.get("navy_game_id"):
