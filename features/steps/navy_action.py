...
def step_impl(context,ship_name,pos_x,pos_y,course):
    ...
    context.ship = ship_service.add(test_utils.json_ship(ship_name,pos_x,pos_y,course,context.user.id,context.game.id))
...
def step_impl(context):
    ...
    context.page = context.client.post(
        url_for("navy.action"), json=test_utils.json_action(context.user.id,context.ship.course,1,4,context.ship.missile_type_id,context.ship.id,3), headers=context.headers
    )
...
def step_impl(context):
    ...
    context.page = context.client.post(
        url_for("navy.action"), json=test_utils.json_action(context.user.id,context.ship.course,1,context.game.id,context.ship.missile_type_id,context.ship.id,2), headers=context.headers
    )
