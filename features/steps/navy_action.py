...
@when("the user '{user_id:d}' turns his ship to '{course}', moves it '{distance:d}' cells and attacks for round '{round:d}' in NavyGame '{game_id:d}'")
def step_impl(context, user_id, course, distance, round, game_id):
    headers = test_utils.get_header(context.tokens[user_id]) 
    current_ship = context.ships[user_id]
    body = test_utils.json_action(user_id, course, 1, game_id, current_ship.missile_type_id, current_ship.id, distance, round)
    context.pages[user_id] = context.client.post(
        url_for("navy.action"), json=body, headers=headers
    )
    
    
@when("the user '{user1_id:d}' turns users '{user2_id:d}' ship to '{course}' and moves it '{distance:d}' cells for round '{round:d}' in NavyGame '{game_id:d}'")
def step_impl(context, user1_id, user2_id, course, distance, round, game_id):
    ...
    body = test_utils.json_action(user1_id, course, 1, game_id, current_ship.missile_type_id, current_ship.id, distance, round)
