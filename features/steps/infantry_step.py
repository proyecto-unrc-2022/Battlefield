...
def step_impl(context):
    user2 = db.session.query(User).filter_by(username= "Tomas").first()
    game = Game_Infantry(id_user1= user2.id, id_user2= None)
    db.session.add(game)
    db.session.commit()  
...
def step_impl(context):
    context.page = context.client.post(url_for("infantry.choose_figure", game_id = 1, user_id = 2, type= 2))
    assert context.page.status_code == 200
    
@then('the humvee is created')
def step_impl(context):
    user2 = User.query.filter_by(username= "Tomas").first()
    print(user2.id)
    ob = Figure_infantry.query.filter_by(id_user = user2.id, id_game = 1).first()
    context.ob = ob
...
def step_given(context):
    user3 = User(username= "Nicolas", email="Nicolas@gmail.com", password="123")
    add_user(user3.username, user3.password, user3.email)
    context.user = user3
    assert True
...
def step_when(context):
    ...
    context.page = context.client.post(url_for("infantry.shoot_entity",direction= EAST, figure_id=1, game_id= 1))
    assert context.page.status_code == 200
