...
@given("a missile exists from user '{user_id:d}' in '{pos_x:d}', '{pos_y:d}' with course '{course}', speed '{speed:d}', and damage '{damage:d}' in the NavyGame '{game_id:d}'")
def step_impl(context, user_id, pos_x, pos_y, course, speed, damage, game_id):
    ...
    missile_db = test_utils.add_test_missile(course, pos_x, pos_y, context.ships[user_id].id, game_id, context.ships[user_id].missile_type_id, damage, speed)
...
@then("a missile with course '{course}', speed '{speed:d}', and damage '{damage:d}' should be in '{pos_x:d}', '{pos_y:d}' in the NavyGame '{game_id:d}'")
