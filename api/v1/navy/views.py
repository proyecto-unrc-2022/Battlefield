...
def new_navy_game():
    try:
        ...
        return NavyResponse(201, data=NavyGameDTO().dump(created_game), message="Game created.").to_json(), 201
    except ValidationError as err:
        return jsonify(err.messages), 400
...
def delete_navy_game(id):
    ...
    return NavyResponse(status=200, data=NavyGameDTO().dump(deleted_game), message="Game deleted.").to_json(), 200 